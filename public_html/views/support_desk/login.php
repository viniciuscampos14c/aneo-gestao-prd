<section class="grid w-full gap-8 lg:grid-cols-[1.1fr_0.9fr]">
    <div class="rounded-3xl border border-blue-700/60 bg-gradient-to-br from-blue-900/70 to-cyan-900/30 p-8 shadow-2xl shadow-blue-950/40">
        ...
        <h1 class="mt-3 text-3xl font-semibold leading-tight">Central Técnica de Chamados</h1>
        <p class="mt-3 text-sm text-blue-100/90">
            Portal isolado para a equipe técnica atender, comentar e atualizar os chamados abertos pelo administrativo usando usuários cadastrados no módulo Usuários.
        </p>
    ...
    <div class="rounded-3xl border border-blue-700/60 bg-blue-950/70 p-8 shadow-2xl shadow-blue-950/30">
        ...
        <p class="mt-1 text-sm text-blue-200/80">Acesso exclusivo da equipe técnica.</p>

        <?php if (!$enabled): ?>
            <div class="mt-4 rounded-lg border border-amber-400/50 bg-amber-500/20 px-4 py-3 text-sm text-amber-100">
                Portal técnico desativado no `config.php` (`support_desk.enabled = false`).
            </div>
        <?php endif; ?>
        ...
        <form action="support.php?route=support/login" method="post" class="mt-6 space-y-4">
            ...
            <label class="block">
                <span class="mb-1 block text-sm text-blue-100">Usuário ou e-mail</span>
                <input
                    type="text"
                    name="username"
                    required
