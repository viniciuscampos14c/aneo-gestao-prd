            ...
            <tbody>
                <?php foreach ($rows as $row): ?>
                    <tr class="border-b border-slate-100 align-top hover:bg-slate-50">
                        ...
                        <td class="px-3 py-3"><?= e((string) ($roles[(string) ($row['user_role'] ?? '')] ?? ucfirst((string) ($row['user_role'] ?? '-')))); ?></td>
                        <td class="px-3 py-3"><?= e($companyName($row)); ?></td>
                        <td class="px-3 py-3"><?= e((string) ($row['module'] ?? '-')); ?></td>
                        <td class="px-3 py-3"><?= e((string) ($row['action'] ?? '-')); ?></td>
                        <td class="px-3 py-3">
                        ...
                        <td class="px-3 py-3">
                            <details>
                                <summary class="cursor-pointer text-xs font-semibold text-cyan-700">Ver detalhes</summary>
                                <pre class="mt-2 max-w-[520px] whitespace-pre-wrap rounded border border-slate-200 bg-slate-50 p-2 text-[11px] leading-relaxed text-slate-700"><?= e($prettyJson((string) ($row['changes_json'] ?? '{}'))); ?></pre>
                            </details>
                        </td>
                    </tr>
                <?php endforeach; ?>
