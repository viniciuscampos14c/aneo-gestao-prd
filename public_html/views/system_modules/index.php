...
$manifestExample = <<<'JSON'
{
  ...
  "author": "ANEO",
  "description": "Módulo oficial instalado pelo painel administrativo.",
  "permissions": [
...
<section class="system-modules-page space-y-6">
    <div class="system-modules-hero-shell overflow-hidden rounded-[1.75rem] border border-cyan-200/50 bg-slate-950 shadow-xl">
        <div class="system-modules-hero grid gap-6 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,.28),transparent_36%),linear-gradient(135deg,#081629,#0e4f63)] p-6 text-white lg:grid-cols-[1.4fr_.9fr]">
            <div>
                ...
                <h2 class="system-modules-hero-title mt-2 text-3xl font-semibold">Módulos do Sistema</h2>
                <p class="system-modules-hero-copy mt-2 max-w-3xl text-sm leading-relaxed text-cyan-50/80">
                    Instale pacotes oficiais sem sobrescrever o core. Cada módulo fica isolado, com manifesto, logs,
    ...
    <?php if (!$zipAvailable): ?>
        <div class="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
            A extensão ZIP do PHP não esta habilitada. O upload de instaladores depende dela.
        </div>
    <?php endif; ?>
    ...
    <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <form method="post" action="<?= route('system-modules/upload'); ?>" enctype="multipart/form-data" class="grid gap-3 md:grid-cols-[1fr_auto]">
                ...
                <label class="block">
                    <span class="mb-1 block text-sm font-medium text-slate-700">Pacote do módulo</span>
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
                <div class="rounded-xl border border-cyan-100 bg-cyan-50/70 p-3 text-xs text-cyan-800">
                    <p class="font-semibold">1. Validação</p>
                ...
                <div class="rounded-xl border border-emerald-100 bg-emerald-50/70 p-3 text-xs text-emerald-800">
                    ...
                    <p class="mt-1">Registra logs, migrations e deixa o módulo inativo por seguranca.</p>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs leading-relaxed text-slate-600">
                ...
                <p class="mt-3">Módulos ativos podem declarar rotas em <code>routes.php</code>. As rotas precisam iniciar com <code>modules/chave_do_modulo</code>.</p>
            </div>
        ...
        <div class="border-b border-slate-100 p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <h3 class="mt-1 text-lg font-semibold text-slate-800">Módulos instalados</h3>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($modules as $module): ?>
                        ...
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-4 py-3">
                                <a href="<?= route('system-modules&id=' . $moduleId); ?>" class="font-semibold text-slate-800 hover:text-cyan-600"><?= e((string) ($module['title'] ?? 'Módulo')); ?></a>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap justify-end gap-2">
                                    ...
                                    <?php if ($status === 'active'): ?>
                                        <form method="post" action="<?= route('system-modules/deactivate'); ?>" onsubmit="return confirm('Desativar este módulo?');">
                                            <input type="hidden" name="_csrf" value="<?= csrf_token(); ?>">
                                    ...
                                    <?php elseif ($status !== 'error'): ?>
                                        <form method="post" action="<?= route('system-modules/activate'); ?>" onsubmit="return confirm('Ativar este módulo?');">
                                            <input type="hidden" name="_csrf" value="<?= csrf_token(); ?>">
                                            <input type="hidden" name="module_id" value="<?= $moduleId; ?>">
                    ...
                    <?php if ($modules === []): ?>
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">
                                Nenhum módulo instalado ainda. Envie o primeiro pacote oficial para iniciar o inventario.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm text-slate-500"><?= e((string) ($selectedModule['description'] ?? 'Sem descrição.')); ?></p>
                    </div>
                ...
                <div class="mt-5 grid gap-4 lg:grid-cols-2">
                    <div>
                        ...
                        <div class="mt-2 space-y-2">
                            ...
                            <?php endforeach; ?>
                            <?php if ($permissions === []): ?>
                                <p class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500">Nenhuma permissão declarada.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div>
    ...
    <div class="grid gap-5 xl:grid-cols-2">
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 p-5">
                ...
                <p class="text-sm text-slate-500">Histórico de SQL executado pelos pacotes instalados.</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
